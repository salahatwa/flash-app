import { AfterViewInit, Component, ElementRef, forwardRef, Inject, Injectable, Input, Renderer2, ViewChild } from '@angular/core';
...
@Component({
  ...
  template: '<input type="file" id="fileUpload" name="fileUpload" accept="image/*" style="display:none;" /> <textarea #markdownEditorElm id="{{textareaId}}" name="{{textareaId}}" data-provide="markdown" rows="{{rows}}"></textarea>',
...
export class AngularMarkdownEditorComponent implements AfterViewInit {
  ...
  constructor(@Inject('config') private forRootConfig: EditorOption, private renderer: Renderer2) { }
